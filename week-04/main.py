...
RL_LOGIN_SIGNUP_LIMIT = int(os.getenv("RL_LOGIN_SIGNUP_LIMIT", "5"))
RL_LOGIN_SIGNUP_WINDOW = int(os.getenv("RL_LOGIN_SIGNUP_WINDOW", "60"))  # seconds
RL_SHORTEN_LIMIT = int(os.getenv("RL_SHORTEN_LIMIT", "30"))
RL_SHORTEN_WINDOW = int(os.getenv("RL_SHORTEN_WINDOW", "60"))

# --------------------------------------------------------------------------------------
# App + dirs
# --------------------------------------------------------------------------------------
app = FastAPI(title="TinyLink API", version="4.0")
...
class Url(SQLModel, table=True):
    ...
    # Week-4 fix: admin search referenced tags; add it to avoid runtime errors
...
# --------------------------------------------------------------------------------------
# Simple in-memory Rate Limiter (Week-4)
# --------------------------------------------------------------------------------------
class RateLimiter:
    """
    Sliding window, per-key limiter with deque of timestamps (UTC seconds).
    Not distributed; suitable for single-process dev or small prod.
    """
    def __init__(self):
...
def rate_limit_dep(limit: int, window_secs: int):
    async def _inner(request: Request):
        ip = request.client.host if request.client else "unknown"
        key = f"{ip}:{request.url.path}"
        if not rate_limiter.allow(key, limit, window_secs):
...
class StatsOut(BaseModel):
    ...
    referrers: List[Dict[str, Any]]  # Week-4 addition
...
# --------------------------------------------------------------------------------------
# Auth endpoints (API + form) — with rate limiting (Week-4)
# --------------------------------------------------------------------------------------
@app.post("/auth/signup", response_model=TokenOut, dependencies=[Depends(rate_limit_dep(RL_LOGIN_SIGNUP_LIMIT, RL_LOGIN_SIGNUP_WINDOW))])
...
@app.post("/auth/login", response_model=TokenOut, dependencies=[Depends(rate_limit_dep(RL_LOGIN_SIGNUP_LIMIT, RL_LOGIN_SIGNUP_WINDOW))])
...
@app.post("/auth/login_form", dependencies=[Depends(rate_limit_dep(RL_LOGIN_SIGNUP_LIMIT, RL_LOGIN_SIGNUP_WINDOW))])
def auth_login_form(
    response: Response,
    username: str = Form(...),
    password: str = Form(...),
...
@app.post("/auth/signup_form", dependencies=[Depends(rate_limit_dep(RL_LOGIN_SIGNUP_LIMIT, RL_LOGIN_SIGNUP_WINDOW))])
...
@app.post("/api/shorten", response_model=UrlOut, status_code=status.HTTP_201_CREATED,
          dependencies=[Depends(rate_limit_dep(RL_SHORTEN_LIMIT, RL_SHORTEN_WINDOW))])
def api_shorten(
...
def _log_click(session: Session, url: Url, request: Request) -> None:
    try:
        ...
        # Week-4: capture UTM params from current request if present
        # (if you bounce through this short link with ?utm_source=... they'll be recorded)
        meta: Dict[str, Any] = {}
        ...
        session.add(ClickLog(url_id=url.id, referrer=ref, user_agent=ua, ip=ip, meta=meta_str))
        # no separate commit
...
# Public stats API (Week-4: add referrer breakdown)
...
def stats(code: str, session: Session = Depends(get_session)):
    ...
    for cl in logs:
        key = cl.referrer or "(none)"
        refcounts[key] += 1
    referrers = [{"referrer": k, "count": v} for k, v in sorted(refcounts.items(), key=lambda x: -x[1])]

    return StatsOut(code=code, total_clicks=url.clicks, last_30_days=series, referrers=referrers)

# HTML form shorten (kept)
@app.post("/shorten", response_class=HTMLResponse, dependencies=[Depends(rate_limit_dep(RL_SHORTEN_LIMIT, RL_SHORTEN_WINDOW))])
def shorten_form(
    request: Request,
    long_url: str = Form(...),
